...
print_stackframe(void) {
      ...
      */
    uint32_t ebp = read_ebp(); //获取当前stack frame的base pointer
    uint32_t eip = read_eip();  //call read_eip这条指令下一条指令的地址
    for(int i=0;i<STACKFRAME_DEPTH&&ebp!=0;i++)//之前没有写ebp!=0
    {
        cprintf("ebp:0x%08x eip:0x%08x",ebp,eip);
        uint32_t *args=(uint32_t*)(ebp+8); //参数存放在ebp+8指向的内存上
        cprintf(" args: " );
        for(int j=0;j<4;j++)
        {
        ...
        eip=*((uint32_t*)(ebp+4));
        ebp=*((uint32_t*)ebp); //ebp中的地址处总是“上一层函数调用时的ebp值”
    }
}
